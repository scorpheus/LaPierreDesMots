/**
 * `MoteurChrono` — le composant hôte du moteur `chrono`. Lot L2-E.
 *
 * Il ne décide de RIEN. Toute la règle vit dans `moteurChrono` (paquet `partage`) ; ce
 * composant fait quatre choses et pas une de plus : traduire un geste en `ActionChrono`,
...
export function MoteurChrono(
  ...
  return (
    ...
    >
      ...
      <div data-plateau="vignettes" style={{ display: 'flex', flexWrap: 'wrap' }}>
        {contenu.vignettes.map((vignette) => (
          <button
            key={vignette.id}
            type="button"
            data-vignette={vignette.id}
            data-numero={etat.acquis[vignette.id] ?? ''}
            style={{ ...STYLE_CIBLE, minWidth: CIBLE_PX * 2 }}
            onClick={(evenement) => {
              jouer({ type: 'numeroter', vignette: vignette.id }, evenement);
            }}
          >
            {vignette.libelle}
          </button>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={STYLE_CIBLE}
          onClick={() => {
            emettre({ type: 'ecouterConsigne' } as ActionChrono);
          }}
        >
          Écouter
        </button>
        ...
        <button
          ...
          style={STYLE_CIBLE}
          onClick={() => {
            emettre({ type: 'demanderAide' } as ActionChrono);
          }}
        >
          Gobi, aide-moi
        </button>
