/**
 * `MoteurPaires` — le composant hôte du moteur `paires`. Lot L2-E.
 *
 * Il ne décide de RIEN. Toute la règle vit dans `moteurPaires` (paquet `partage`) ; ce
 * composant fait quatre choses et pas une de plus : traduire un geste en `ActionPaires`,
...
export function MoteurPaires(
  ...
  return (
    ...
    >
      ...
      <div data-plateau="cartes" style={{ display: 'flex', flexWrap: 'wrap' }}>
        {contenu.cartes.map((carte) => (
          <button
            key={carte.id}
            type="button"
            data-carte={carte.id}
            data-face={carte.face}
            data-retournee={etat.carteRetournee === carte.id ? 'oui' : 'non'}
            data-appariee={etat.acquis[carte.paire] === undefined ? 'non' : 'oui'}
            style={STYLE_CIBLE}
            onClick={(evenement) => {
