/**
 * `MoteurPhrase` — le composant hôte du moteur `phrase`. Lot L2-E.
 *
 * Il ne décide de RIEN. Toute la règle vit dans `moteurPhrase` (paquet `partage`) ; ce
 * composant fait quatre choses et pas une de plus : traduire un geste en `ActionPhrase`,
...
export function MoteurPhrase(
  ...
  return (
    ...
    >
      ...
      <div data-plateau="phrase" style={{ display: 'flex', flexWrap: 'wrap' }}>
        {(consigne === null ? [] : consigne.ordre).map((id) => {
          const etiquette = contenu.etiquettes.find((e) => e.id === id);
          ...
          return (
            <span
              ...
              style={{ ...STYLE_CIBLE, display: 'inline-block' }}
            >
              {placee && etiquette !== undefined ? etiquette.mot : '\u00a0'}
            </span>
          );
        })}
      </div>

      <div data-plateau="etiquettes" style={{ display: 'flex', flexWrap: 'wrap' }}>
        {contenu.etiquettes.map((etiquette) => (
          <button
            key={etiquette.id}
            type="button"
            data-etiquette={etiquette.id}
            data-placee={etat.acquis[etiquette.id] === undefined ? 'non' : 'oui'}
            style={STYLE_CIBLE}
            onClick={(evenement) => {
              jouer({ type: 'placer', etiquette: etiquette.id }, evenement);
            }}
          >
            {etiquette.mot}
          </button>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
        ...
        <button
          ...
          style={STYLE_CIBLE}
          onClick={() => {
            emettre({ type: 'ecouterConsigne' } as ActionPhrase);
          }}
        >
          Écouter
        </button>
        ...
        <button
          ...
          style={STYLE_CIBLE}
          onClick={() => {
            emettre({ type: 'demanderAide' } as ActionPhrase);
          }}
        >
          Gobi, aide-moi
        </button>
