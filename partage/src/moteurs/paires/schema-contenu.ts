/**
 * Schéma JSON 2020-12 du bloc `jeu.contenu` pour le moteur `paires` — lot L2-E.
 *
 ...
 *
 * Deux cartes par valeur de `paire` : le schéma ne sait pas compter, `test:contenu` le fera.
 * Une paire orpheline rendrait l'exercice interminable — un état sans issue (test `singe`).
 */
...
export const SCHEMA_CONTENU_PAIRES: SchemaJson = {
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  $id: 'pierre:schemas/contenu-paires',
  type: 'object',
  additionalProperties: false,
  required: ['consignes', 'cartes', 'competence'],
  properties: {
    consignes: {
      type: 'array',
      minItems: 1,
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['id', 'texte', 'forme', 'audio', 'aApparier', 'motsCles'],
        properties: {
          id: ID_CONSIGNE,
          texte: TEXTE,
          forme: { enum: ['imperative', 'affirmative'] },
          audio: { type: ['string', 'null'] },
          aApparier: { type: 'array', minItems: 1, maxItems: 8, uniqueItems: true, items: ID },
          motsCles: MOTS_CLES,
        },
      },
    },
    cartes: {
      ...
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['id', 'libelle', 'face', 'asset', 'paire'],
        properties: {
          id: ID,
          libelle: LIBELLE,
          face: { enum: ['mot', 'image'] },
