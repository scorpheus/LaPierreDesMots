/**
 * `moteurAttrape` — la mécanique pure de `attrape`. Lot L2-E.
 *
 * Toucher les bonnes cibles mobiles parmi des intrus. Seule la LECTURE de l'étiquette
 * fait la difficulté : la trajectoire appartient à l'habillage, jamais à la règle.
 *
...
import { SCHEMA_CONTENU_ATTRAPE } from './schema-contenu.js';
import { evaluerAttrape, modeReponseAttrape } from './validation.js';
import type { DecisionAttrape } from './validation.js';
import type {
  ActionAttrape,
  ContenuAttrape,
  EtatAttrape,
...
function remplacer(
  etapes: readonly EtatEtapeAttrape[],
  index: number,
  remplacante: EtatEtapeAttrape,
): readonly EtatEtapeAttrape[] {
 ...
 */
function appliquerPaliers(etat: EtatAttrape, instant: number): EtatAttrape {
...
function appliquerDecision(
  etat: EtatAttrape,
  decision: DecisionAttrape,
  refus: RefusAttrape,
  instant: number,
  extra: Partial<EtatAttrape>,
): EtatAttrape {
  const etape = etat.etapes[etat.indexEtape];
  if (etape === undefined) return etat;
  ...
  if (!decision.acceptee) {
    ...
    const majEtape: EtatEtapeAttrape = {
      ...etape,
...
export const moteurAttrape: Moteur<ContenuAttrape, EtatAttrape, ActionAttrape> = {
  ...
  schemaContenu: SCHEMA_CONTENU_ATTRAPE,

  creerEtat(entree: EntreeMoteur<ContenuAttrape>): EtatAttrape {
    const instant = entree.horloge.maintenantMs();
    const etapes = entree.contenu.consignes.map(
      (etape, index): EtatEtapeAttrape => ({
  ...
  reduire(etat: EtatAttrape, action: ActionAttrape, contexte: ContexteMoteur): EtatAttrape {
    ...
    switch (action.type) {
      case 'toucher': {
        ...
        return appliquerDecision(
          etat,
          decision,
          { cible: action.cible, motif: decision.motif ?? 'cible-inconnue', instantMs: instant },
          instant,
          {},
        );
      }
  ...
  progression(etat: EtatAttrape): ProgressionMoteur {
    return progressionDepuisEtapes(etat.etapes, etat.indexEtape);
  },

  aideProposee(etat: EtatAttrape): AideProposee | null {
    return etat.aide;
  },

  resume(etat: EtatAttrape): ResumeTentative {
    const fin =
      etat.termineMs ??
