/**
 * Schéma JSON 2020-12 du bloc `jeu.contenu` pour le moteur `chemin` — lot L2-E.
 *
 * Validation en deux temps (contrat v1 § 9.1) : l'enveloppe est validée par
 * `contenu/schemas/exercice.schema.json`, qui déclare `jeu.contenu` à `true` ; c'est CE
 * schéma-ci, publié par le moteur lui-même, qui valide le bloc.
 *
 * `voisines` doit être SYMÉTRIQUE ; le schéma ne sait pas l'exprimer, `test:contenu` le
 * vérifiera. Un plateau asymétrique ferait un cul-de-sac invisible — le pire bug possible.
 */

import type { SchemaJson } from '../types.js';

/** Identifiant d'objet de contenu, même forme que partout ailleurs dans le dépôt. */
const ID = { type: 'string', pattern: '^[a-z0-9]+(-[a-z0-9]+)*$' } as const;
const ID_CONSIGNE = { type: 'string', pattern: '^c[0-9]+$' } as const;
const LIBELLE = { type: 'string', minLength: 1, maxLength: 80 } as const;
const TEXTE = { type: 'string', minLength: 3, maxLength: 200 } as const;
const MOTS_CLES = { type: 'array', minItems: 1, items: { type: 'string', minLength: 2 } } as const;
const POINT = {
  type: 'array',
  minItems: 2,
  maxItems: 2,
  items: { type: 'number' },
} as const;

export const SCHEMA_CONTENU_CHEMIN: SchemaJson = {
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  $id: 'pierre:schemas/contenu-chemin',
  type: 'object',
  additionalProperties: false,
  required: ['consignes', 'cases', 'competence'],
  properties: {
    consignes: {
      type: 'array',
      minItems: 1,
      maxItems: 6,
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['id', 'texte', 'forme', 'audio', 'depart', 'parcours', 'motsCles'],
        properties: {
          id: ID_CONSIGNE,
          texte: TEXTE,
          forme: { enum: ['imperative', 'affirmative'] },
          audio: { type: ['string', 'null'] },
          depart: ID,
          parcours: { type: 'array', minItems: 2, maxItems: 12, uniqueItems: true, items: ID },
          motsCles: MOTS_CLES,
        },
      },
    },
    cases: {
      type: 'array',
      minItems: 3,
      maxItems: 40,
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['id', 'libelle', 'position', 'voisines', 'confusionAvec'],
        properties: {
          id: ID,
          libelle: LIBELLE,
          position: POINT,
          voisines: { type: 'array', minItems: 1, uniqueItems: true, items: ID },
          confusionAvec: { type: ['string', 'null'] },
        },
      },
    },
    competence: { type: 'string', minLength: 3 },
  },
};
