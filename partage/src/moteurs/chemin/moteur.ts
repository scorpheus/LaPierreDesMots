/**
 * `moteurChemin` — la mécanique pure de `chemin`. Lot L2-E.
 *
 * Tracer une route en enchaînant les bonnes cases. Une case hors de portée est ignorée,
 * jamais refusée : le geste imprécis ne coûte rien (R16).
 *
...
function remplacer(
  etapes: readonly EtatEtapeChemin[],
  index: number,
  remplacante: EtatEtapeChemin,
): readonly EtatEtapeChemin[] {
 ...
 */
function appliquerPaliers(etat: EtatChemin, instant: number): EtatChemin {
...
function appliquerDecision(
  etat: EtatChemin,
  decision: DecisionChemin,
  refus: RefusChemin,
  instant: number,
  extra: Partial<EtatChemin>,
): EtatChemin {
  const etape = etat.etapes[etat.indexEtape];
  if (etape === undefined) return etat;
  ...
  if (!decision.acceptee) {
    ...
    const majEtape: EtatEtapeChemin = {
      ...etape,
...
export const moteurChemin: Moteur<ContenuChemin, EtatChemin, ActionChemin> = {
  code: 'chemin',
  version: 1,
  capacites: {
  ...
  schemaContenu: SCHEMA_CONTENU_CHEMIN,

  creerEtat(entree: EntreeMoteur<ContenuChemin>): EtatChemin {
    const instant = entree.horloge.maintenantMs();
    const etapes = entree.contenu.consignes.map(
      (etape, index): EtatEtapeChemin => ({
        identifiant: etape.id,
        restantes: [...etape.parcours],
        nbErreurs: 0,
        niveauAide: 'aucune',
        nbEcoutes: 0,
    ...
    return {
      indexEtape: 0,
      etapes,
      cases: [...entree.contenu.cases],
      departs: entree.contenu.consignes.map((c) => c.depart),
  ...
  reduire(etat: EtatChemin, action: ActionChemin, contexte: ContexteMoteur): EtatChemin {
    const instant = contexte.horloge.maintenantMs();
    const etape = etat.etapes[etat.indexEtape];

    switch (action.type) {
      case 'avancer': {
        const decision = evaluerChemin(etat, action.caseVisee);
        const suivant = appliquerDecision(
          etat,
          decision,
          {
            caseVisee: action.caseVisee,
            motif: decision.motif ?? 'case-inconnue',
            instantMs: instant,
          },
          instant,
  ...
  progression(etat: EtatChemin): ProgressionMoteur {
    return progressionDepuisEtapes(etat.etapes, etat.indexEtape);
  },

  aideProposee(etat: EtatChemin): AideProposee | null {
    return etat.aide;
  },

  resume(etat: EtatChemin): ResumeTentative {
    const fin =
      etat.termineMs ??
