/**
 * `moteurChrono` — la mécanique pure de `chrono`. Lot L2-E.
 *
 * Remettre des vignettes dans l'ordre du récit — « Numérote de 1 à 5 » du niveau 5.
 *
...
import { SCHEMA_CONTENU_CHRONO } from './schema-contenu.js';
import { evaluerChrono, modeReponseChrono } from './validation.js';
import type { DecisionChrono } from './validation.js';
import type {
  ActionChrono,
  ContenuChrono,
  EtatChrono,
...
function remplacer(
  etapes: readonly EtatEtapeChrono[],
  index: number,
  remplacante: EtatEtapeChrono,
): readonly EtatEtapeChrono[] {
 ...
 */
function appliquerPaliers(etat: EtatChrono, instant: number): EtatChrono {
...
function appliquerDecision(
  etat: EtatChrono,
  decision: DecisionChrono,
  refus: RefusChrono,
  instant: number,
  extra: Partial<EtatChrono>,
): EtatChrono {
  const etape = etat.etapes[etat.indexEtape];
  if (etape === undefined) return etat;
  ...
  if (!decision.acceptee) {
    ...
    const majEtape: EtatEtapeChrono = {
      ...etape,
...
export const moteurChrono: Moteur<ContenuChrono, EtatChrono, ActionChrono> = {
  code: 'chrono',
  version: 1,
  capacites: {
  ...
  schemaContenu: SCHEMA_CONTENU_CHRONO,
  ...
  creerEtat(entree: EntreeMoteur<ContenuChrono>): EtatChrono {
    ...
    const etapes = entree.contenu.consignes.map(
      (etape, index): EtatEtapeChrono => ({
        ...
        modeReponse: modeReponseChrono(entree.contenu),
  ...
  reduire(etat: EtatChrono, action: ActionChrono, contexte: ContexteMoteur): EtatChrono {
    const instant = contexte.horloge.maintenantMs();
    const etape = etat.etapes[etat.indexEtape];

    switch (action.type) {
      case 'numeroter': {
        const decision = evaluerChrono(etat, action.vignette);
        return appliquerDecision(
          etat,
          decision,
          {
            vignette: action.vignette,
            motif: decision.motif ?? 'vignette-inconnue',
            instantMs: instant,
          },
          instant,
          {},
  ...
  progression(etat: EtatChrono): ProgressionMoteur {
    return progressionDepuisEtapes(etat.etapes, etat.indexEtape);
  },

  aideProposee(etat: EtatChrono): AideProposee | null {
    return etat.aide;
  },

  resume(etat: EtatChrono): ResumeTentative {
    const fin =
      etat.termineMs ??
