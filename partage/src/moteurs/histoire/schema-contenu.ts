/**
 * Schéma JSON 2020-12 du bloc `jeu.contenu` pour le moteur `histoire` — lot L2-E.
 *
 * Validation en deux temps (contrat v1 § 9.1) : l'enveloppe est validée par
 * `contenu/schemas/exercice.schema.json`, qui déclare `jeu.contenu` à `true` ; c'est CE
 * schéma-ci, publié par le moteur lui-même, qui valide le bloc.
 *
 * `options` va de 2 à 4 : DEUX, c'est le vrai/faux, qui n'a pas de moteur à lui (§ 8, n° 6).
 * C'est le nombre d'options qui fixe `p_devinette`, question par question (D13).
 */

import type { SchemaJson } from '../types.js';

/** Identifiant d'objet de contenu, même forme que partout ailleurs dans le dépôt. */
const ID = { type: 'string', pattern: '^[a-z0-9]+(-[a-z0-9]+)*$' } as const;
const ID_CONSIGNE = { type: 'string', pattern: '^c[0-9]+$' } as const;
const LIBELLE = { type: 'string', minLength: 1, maxLength: 80 } as const;
const TEXTE = { type: 'string', minLength: 3, maxLength: 200 } as const;
const MOTS_CLES = { type: 'array', minItems: 1, items: { type: 'string', minLength: 2 } } as const;
const POINT = {
  type: 'array',
  minItems: 2,
  maxItems: 2,
  items: { type: 'number' },
} as const;

export const SCHEMA_CONTENU_HISTOIRE: SchemaJson = {
  $schema: 'https://json-schema.org/draft/2020-12/schema',
  $id: 'pierre:schemas/contenu-histoire',
  type: 'object',
  additionalProperties: false,
  required: ['titre', 'recit', 'audioRecit', 'questions', 'options', 'competence'],
  properties: {
    titre: LIBELLE,
    // 600 caractères : un texte court, tenu sur un écran, sans défilement pendant la lecture.
    recit: { type: 'string', minLength: 20, maxLength: 600 },
    audioRecit: { type: ['string', 'null'] },
    questions: {
      type: 'array',
      minItems: 1,
      maxItems: 8,
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['id', 'texte', 'audio', 'options', 'reponse', 'motsCles'],
        properties: {
          id: ID_CONSIGNE,
          texte: TEXTE,
          audio: { type: ['string', 'null'] },
          options: { type: 'array', minItems: 2, maxItems: 4, uniqueItems: true, items: ID },
          reponse: ID,
          motsCles: MOTS_CLES,
        },
      },
    },
    options: {
      type: 'array',
      minItems: 2,
      maxItems: 32,
      items: {
        type: 'object',
        additionalProperties: false,
        required: ['id', 'libelle', 'confusionAvec'],
        properties: {
          id: ID,
          libelle: LIBELLE,
          confusionAvec: { type: ['string', 'null'] },
        },
      },
    },
    competence: { type: 'string', minLength: 3 },
  },
};
