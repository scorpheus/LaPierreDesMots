/**
 * `moteurAssemble` — la mécanique pure de `assemble`. Lot L2-E.
 *
 * Faire glisser des blocs-syllabes pour former un mot. L'ordre fait partie de la réponse.
 *
...
import { SCHEMA_CONTENU_ASSEMBLE } from './schema-contenu.js';
import { evaluerAssemble, modeReponseAssemble } from './validation.js';
import type { DecisionAssemble } from './validation.js';
import type {
  ActionAssemble,
  ContenuAssemble,
  EtatAssemble,
...
function remplacer(
  etapes: readonly EtatEtapeAssemble[],
  index: number,
  remplacante: EtatEtapeAssemble,
): readonly EtatEtapeAssemble[] {
 ...
 */
function appliquerPaliers(etat: EtatAssemble, instant: number): EtatAssemble {
...
function appliquerDecision(
  etat: EtatAssemble,
  decision: DecisionAssemble,
  refus: RefusAssemble,
  instant: number,
  extra: Partial<EtatAssemble>,
): EtatAssemble {
  const etape = etat.etapes[etat.indexEtape];
  if (etape === undefined) return etat;
  ...
  if (!decision.acceptee) {
    ...
    const majEtape: EtatEtapeAssemble = {
      ...etape,
...
export const moteurAssemble: Moteur<ContenuAssemble, EtatAssemble, ActionAssemble> = {
  code: 'assemble',
  version: 1,
  capacites: {
  ...
  schemaContenu: SCHEMA_CONTENU_ASSEMBLE,
  ...
  reduire(etat: EtatAssemble, action: ActionAssemble, contexte: ContexteMoteur): EtatAssemble {
    const instant = contexte.horloge.maintenantMs();
    const etape = etat.etapes[etat.indexEtape];

    switch (action.type) {
      case 'poser': {
        const decision = evaluerAssemble(etat, action.bloc);
        return appliquerDecision(
          etat,
          decision,
          { bloc: action.bloc, motif: decision.motif ?? 'bloc-inconnu', instantMs: instant },
          instant,
          {},
        );
      }
  ...
  progression(etat: EtatAssemble): ProgressionMoteur {
    return progressionDepuisEtapes(etat.etapes, etat.indexEtape);
  },

  aideProposee(etat: EtatAssemble): AideProposee | null {
    return etat.aide;
  },

  resume(etat: EtatAssemble): ResumeTentative {
    const fin =
      etat.termineMs ??
