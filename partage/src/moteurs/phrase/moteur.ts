/**
 * `moteurPhrase` — la mécanique pure de `phrase`. Lot L2-E.
 *
 * Ordonner des étiquettes-mots pour reconstruire une phrase.
 *
...
import { SCHEMA_CONTENU_PHRASE } from './schema-contenu.js';
import { evaluerPhrase, modeReponsePhrase } from './validation.js';
import type { DecisionPhrase } from './validation.js';
import type {
  ActionPhrase,
  ContenuPhrase,
  EtatPhrase,
...
function remplacer(
  etapes: readonly EtatEtapePhrase[],
  index: number,
  remplacante: EtatEtapePhrase,
): readonly EtatEtapePhrase[] {
 ...
 */
function appliquerPaliers(etat: EtatPhrase, instant: number): EtatPhrase {
...
function appliquerDecision(
  etat: EtatPhrase,
  decision: DecisionPhrase,
  refus: RefusPhrase,
  instant: number,
  extra: Partial<EtatPhrase>,
): EtatPhrase {
  const etape = etat.etapes[etat.indexEtape];
  if (etape === undefined) return etat;
  ...
  if (!decision.acceptee) {
    ...
    const majEtape: EtatEtapePhrase = {
      ...etape,
...
export const moteurPhrase: Moteur<ContenuPhrase, EtatPhrase, ActionPhrase> = {
  code: 'phrase',
  version: 1,
  capacites: {
  ...
  schemaContenu: SCHEMA_CONTENU_PHRASE,

  creerEtat(entree: EntreeMoteur<ContenuPhrase>): EtatPhrase {
    ...
    return {
      indexEtape: 0,
      etapes,
      etiquettes: [...entree.contenu.etiquettes],
  ...
  reduire(etat: EtatPhrase, action: ActionPhrase, contexte: ContexteMoteur): EtatPhrase {
    const instant = contexte.horloge.maintenantMs();
    const etape = etat.etapes[etat.indexEtape];

    switch (action.type) {
      case 'placer': {
        const decision = evaluerPhrase(etat, action.etiquette);
        return appliquerDecision(
          etat,
          decision,
          {
            etiquette: action.etiquette,
            motif: decision.motif ?? 'etiquette-inconnue',
            instantMs: instant,
          },
          instant,
          {},
  ...
  progression(etat: EtatPhrase): ProgressionMoteur {
    return progressionDepuisEtapes(etat.etapes, etat.indexEtape);
  },

  aideProposee(etat: EtatPhrase): AideProposee | null {
    return etat.aide;
  },

  resume(etat: EtatPhrase): ResumeTentative {
    const fin =
      etat.termineMs ??
