...
Dernière mise à jour du pilotage : 14 septembre 2026.
...
## Pilotage actif — 14 septembre 2026

### Résultat à livrer et choix de reprise

Demande du parent : obtenir un site réellement présentable, avec progression compréhensible,
images bien cadrées et éléments dans le bon ordre sur PC, tablette et téléphone ; arrêter les
reprises et campagnes dispersées. Le parent rapporte aujourd'hui une progression incorrecte,
des images coupées, des adaptations/orientations défectueuses et une superposition sur PC.
Il précise que ces contrôles doivent être automatisés : son inventaire manuel n'est pas un
prérequis pour avancer. Les détails de son écran et de son profil compléteront la reproduction
lorsqu'il les donnera. Aucun de ces défauts n'est déclaré corrigé par ce document.

**Cap retenu pour le pilotage : remise au propre ciblée de l'existant.** Une réécriture totale
n'est étayée par aucune cause actuelle démontrée. Conserver journal, acquis, règles pédagogiques,
moteurs utiles et illustrations validées. Le site web/PWA sert de livrable de référence à cette
finition, sur les trois types d'appareil demandés. LAN et APK restent disponibles ; aucun portage
supplémentaire ni nouvelle fonctionnalité autonome n'est lancé pendant ce lot. Une modification
du socle partagé garde les contrôles de compatibilité concernés.

### Coordination en cours

- Cette tâche, **« Comparer les deux sites »**, assure désormais le pilotage de cette reprise.
  Elle tient cette file et intègre les retours ; elle ne crée pas un nouveau rapport concurrent
  pour chaque écran.
- La tâche **« Résoudre les problèmes de campagne »** conserve le lot PWA du 14 septembre :
  worker, préparation/publication, tests associés, recette de cycle et skill PWA. Son sous-agent
  Luna détenait le jeton unique des suites et constructions pour cette campagne, selon la demande
  explicite du parent dans cette tâche. Le jeton a été explicitement restitué après la clôture
  du 14 septembre à 13 h 58 ; il revient au pilotage pour le prochain lot borné.
- Passation reçue et lue : sept défauts PWA corrigés localement ; 42 tests de famille PWA et
  155 de progression annoncés verts ; recette du vrai worker et recette PWA sur profil isolé
  vertes. La campagne globale du 14 septembre s'est achevée à **13 h 58 Europe/Paris**,
  code 0 en **962,5 secondes**. Le pilotage a relu le rapport daté `2026-09-14T11:58:05.302Z`
  et le journal de début/fin : **15/15 étapes vertes**, 2 745 T1/T2, 915 E2E, 13 visuels,
  321 qualité, 655 contenu, 11 bundle ; rejeu et contrôles finaux verts. Détails et limites :
  [audit PWA du 14 septembre](audit-fiabilite-pwa-2026-09-14.md).
- Base locale : `9946abf` avec le lot PWA non commité. Aucun changement de ce lot n'a été publié.
  La version publique et celle réellement exécutée par le navigateur familial doivent être
  distinguées : leur identité n'a pas été requalifiée dans cette reprise de pilotage.
- Les deux lectures indépendantes progression/composition de cette reprise sont terminées,
  sans écriture de code ni exécution de suites. Les conclusions utiles sont intégrées ci-dessous.

### File unique de finition, dans cet ordre

| ID | Priorité et état | Travail borné | Preuve de sortie attendue |
|---|---|---|---|
| F0 | Validation locale acquise, jeton rendu | Lot PWA et campagne unique clos à 13 h 58 le 14 septembre ; rapport et journal final relus par le pilotage. Corrections non commitées et non publiées. | 15/15 étapes vertes, code 0 ; aucun acquis personnel modifié. Cela ne clôt pas les plaintes de progression ou de présentation et ne prouve pas la mise à jour du navigateur familial. |
| F1 | Bloquant, ouvert | Reproduire la progression vécue sur un même profil : inédit, aide, récompense, carte, fermeture/reprise et rejeu ; inclure la remédiation des Galeries. Reprendre le parcours cumulé existant, sans attendre que le parent énumère chaque symptôme. | Ensemble exact des exercices acquis avant/après ; un crédit pour un nouvel exercice réussi, zéro crédit supplémentaire au rejeu ; aucun acquis retiré ; prochaine activité réellement accessible et destination cohérente. Conserver séparément résultat local et résultat sur l'environnement familial. |
| F2 | Bloquant, ouvert | Faire une passe de composition sur les écrans accessibles du vrai parcours, puis corriger la cause commune des rognages/superpositions observés. Commencer par l'écran signalé dès qu'il est identifié. | Image utile entière, texte lisible, commandes accessibles et ordre des calques correct sur PC, tablette et téléphone, après rotation et avec les réglages de lecture concernés. Des gestes réels contrôlent les cibles ; aucune sonde ne les rend artificiellement visibles par défilement automatique. |
| F3 | Finition visible, ouvert | Reprendre ensemble accueil, carte, campement et retour de récompense. Réutiliser les illustrations validées et proposer une composition cohérente : où aller, ce qui vient d'être acquis, ce qui devient accessible. | Un parcours présenté dans la version candidate, avec hiérarchie visuelle et actions compréhensibles ; revue esthétique groupée du parent. Aucun nouvel univers, catalogue ou moteur nécessaire à ce lot. |
| F4 | Finition parent, ouvert | Rendre accessibles les informations pratiques existantes : sauvegarde, export/import, appareil utilisé, progression et mise à jour. Expliquer le stockage local et l'absence de synchronisation entre appareils. | Un adulte retrouve ces informations dans l'interface ; les messages décrivent le comportement livré. Le statut/version PWA est coordonné avec le lot F0, sans activation forcée pendant une partie. |
| F5 | Clôture, après F1–F4 | Stabiliser un état intégré, exécuter sa recette finale et préparer un seul livrable présentable. | Rapport complet de cet état, vérification du parcours dans le livrable, liste explicite des limites et absence de défaut bloquant connu. Publication après validation explicite du livrable préparé ; ensuite contrôle de la version réellement servie. |

Préparation F1 : le parcours navigateur cumulé existant joue deux exercices de la Clairière,
revient à la carte, recharge puis vérifie un nouveau départ inédit. Il interdit l'injection de
réponses et vérifie les acquis exacts. Cette preuve est utile et vient de passer dans F0 ; elle
ne doit pas être rejouée à l'identique pour prétendre contrôler un déblocage de région ou un
palier de cinq exercices. Le prochain contrôle doit couvrir la transition encore absente,
sur le même profil et avec les compteurs exacts, en réutilisant ce parcours.

Les dettes présentes dans l'historique ne disparaissent pas : la file ci-dessus les regroupe par
résultat utilisateur. Une nouvelle observation complète une ligne ou ouvre un défaut identifié
sous cette ligne ; elle ne déclenche pas automatiquement une nouvelle campagne entière.

### Simplification technique à viser

- **Composition : une autorité par scène.** Les composants et `global.css` définissent aujourd'hui
  simultanément dimensions, orientations et ordre, avec plusieurs reprises `!important`.
  Exemple : règles de rétrécissement, placement et portrait dans `global.css`, autour des lignes
  3175–3290 à `9946abf`. Partir d'un défaut reproduit, regrouper ses règles et employer la même
  transformation pour illustration et zones interactives. Étendre seulement aux scènes ayant
  le même besoin. Pas de nouvelle bibliothèque de mise en page ni de réécriture des moteurs.
- **Raster, SVG et Canvas : conserver leurs rôles.** Illustration raster, formes/commandes SVG,
  peinture Canvas sur masque sont compatibles. La carte utilise déjà des ancres communes ;
  `emplacements.ts` mutualise les transformations. Le mot `cover` ou `slice` ne suffit pas à
  prouver un rognage : vérifier les ratios et le rectangle rendu. Référence de la décision :
  [coloriage raster indexé](coloriage-raster-indexe-2026-09-02.md).
- **Progression : expliquer et vérifier les unités.** Maîtrise, étoiles, exercices uniques,
  formes et stades de Gobi n'avancent pas au même rythme. La règle reste un crédit par nouvel
  exercice réussi et cinq crédits par forme. Le passage éventuel à trois reste un arbitrage
  pédagogique distinct. La correction de remédiation des Galeries est présente dans le code ;
  cela ne prouve pas que la plainte actuelle est résolue. L'ancienne mesure « 70/76 » sur cinq
  graines ne démontre pas six blocages actuels et ne doit pas être reprise comme telle.

### Boucle de vérification et coût

La campagne complète du **14 septembre**, relue après sa fin, dure 962,5 secondes :
595,6 secondes de parcours et 269,5 secondes de qualité, contre environ 3,8 et 4,5 secondes
pour les deux constructions et 74 secondes pour T1/T2. Les deux étapes navigateur représentent
donc environ **90 %** de la durée. Aucun gain de temps n'est annoncé par cette réorganisation.

1. Pour chaque correction, nommer le défaut et le contrôle capable de le détecter. Utiliser
   d'abord le cas discriminant, puis la famille touchée, et observer le rendu effectif.
   Commandes existantes : `test:progression`, `test:tactile -- --grep <cas>` et
   `test:responsive -- --grep <cas>`. Ne pas lancer toutes les familles à chaque détail CSS.
2. Les contrôles automatiques doivent détecter rognage, superposition, orientation, cible masquée
   et perte de progression. La présence d'un bouton, une couleur DOM ou un retour HTTP ne
   suffisent pas. Réutiliser et corriger les contrôles existants avant d'ajouter un nouvel outil.
3. Garder `npm run verifier` à la clôture de l'état intégré selon l'annexe T. Après un échec,
   lire sa cause et reprendre son cas ; une relance large demande une modification ou un doute
   précis qui la justifie. Aucun banc de mutation exploratoire dans ce lot.
4. Les crochets existants exécutent encore tout T1/T2 au commit et `verifier` au push. Leurs
   répétitions font partie du coût : les consigner lors de la clôture, sans contournement ni
   promesse de résultat réutilisable avant d'avoir une vérification de fraîcheur correcte.
   Aucun crochet n'est modifié dans la présente reprise de pilotage.
5. Le parent juge la proposition artistique et les règles pédagogiques. Les agents portent
   la détection des défauts mécaniques et de composition, puis présentent une revue groupée.
   Une session sur appareil physique reste distincte d'une simulation de viewport.

### Décisions et limites conservées

Les acquis et sauvegardes personnelles sont préservés ; une reproduction sur données réelles
se fait à partir d'une copie/export, jamais par remise à zéro. Pas de crédit artificiel ni de
modification du rejeu pour débloquer le parcours. Les quatre références originales restent intactes.

La résistance à la fermeture avant accusé de sauvegarde, la cadence trois/cinq, le référentiel
lexical CE1 incomplet, les atlas de compagnons et textes encore en attente restent nommés dans
les documents correspondants. Ils ne sont ni déclarés validés, ni intégrés implicitement par
cette reprise. Les variantes artistiques non approuvées restent en brouillon. Le lot présenté
doit expliciter toute limite encore pertinente ; aucune ligne « tout est fini » sur la seule
base de milliers de tests verts.

## Historique détaillé — conservé pour les preuves, pas comme état du jour
