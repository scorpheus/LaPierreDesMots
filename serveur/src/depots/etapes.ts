 ...
 */
export function journaliserEtapes(
  base: DatabaseSync,
  ...
  const inserer = base.prepare(
    `INSERT OR IGNORE INTO etapes_tentative (${CHAMPS})
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  ...
  for (const etape of etapes) {
    ...
    inserees += Number(resultat.changes);
  }
  return inserees;
}
 ...
 */
export function listerEtapes(base: DatabaseSync, profilId: string): readonly EtapeJournalisee[] {
  const lignes = base
    .prepare(
    ...
    .all(profilId) as unknown as LigneEtape[];
  return lignes.map(versEtape);
}

export function compterEtapes(base: DatabaseSync, profilId: string): number {
  const ligne = base
    .prepare('SELECT COUNT(*) AS n FROM etapes_tentative WHERE profil_id = ?')
 ...
 */
export function compterConfusions(
  base: DatabaseSync,
  profilId: string
): { readonly avecConfusion: number; readonly avecAxe: number } {
  const ligne = base
    .prepare(
      `SELECT
    ...
    .get(profilId) as unknown as { avec_confusion: number | null; avec_axe: number | null };
  return {
