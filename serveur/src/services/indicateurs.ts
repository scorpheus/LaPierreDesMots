/**
 * Les requetes du dashboard parent — contrat des features v2 § 3.8, lot L2-H.
 *
...
import { agregerConfusions, agregerLatences, croiserCouverture } from '@pierre/partage/parent';
 ...
 */
export function latencesDuProfil(base: DatabaseSync, profilId: string): readonly PointLatence[] {
  const lignes = base
    .prepare(
    ...
    .all(profilId) as unknown as LigneLatence[];
 ...
 */
export function confusionsDuProfil(
  base: DatabaseSync,
  profilId: string,
  limite?: number
  ...
  const lignes = base
    ...
    .all(profilId) as unknown as LigneConfusion[];
 ...
 */
export function couvertureDuProfil(
  base: DatabaseSync,
  profilId: string
): readonly CouvertureRegion[] {
  const recoloration = base
    .prepare(
      `SELECT region_code, pourcentage_colorie
       FROM progression_region WHERE profil_id = ?`
    )
    .all(profilId) as unknown as LigneRecoloration[];

  const parCompetence = base
    .prepare(
    ...
    .all(profilId) as unknown as LigneMaitriseRegion[];
