/**
 * Remise à zéro d'un profil — lot H2, côté serveur.
 *
 * ═════════════════════════════════════════════════════════════════════════════════════════
...
import type { Horloge, IdProfil } from '@pierre/partage';
import type {
...
/** Une ligne de `sqlite_master`, telle que `node:sqlite` la rend. */
 ...
 */
export function tablesPorteusesDeProfil(base: DatabaseSync): readonly string[] {
  const tables = base
    .prepare(
  ...
  for (const table of tables) {
    ...
    const colonnes = base.prepare(`PRAGMA table_info(${nom})`).all() as unknown as LigneColonne[];
...
export function previsualiserReinitialisation(
  base: DatabaseSync,
  profilId: string,
  portee: PorteeReinitialisation
): readonly LigneRapportReinitialisation[] {
  return tablesPorteusesDeProfil(base)
    .filter((table) => porteeEfface(portee, table))
    .map((table) => ({
      table,
...
export function reinitialiserProfil(
  ...
  const effectueLe = String(horodatage(horloge));

  const lignes = dansTransaction(base, () => {
    base.exec('PRAGMA defer_foreign_keys = ON;');

    const comptes: LigneRapportReinitialisation[] = [];
    for (const table of tablesPorteusesDeProfil(base)) {
      ...
      const compte = base
        .prepare(`SELECT COUNT(*) AS n FROM ${table} WHERE profil_id = ?`)
        .get(profilId) as unknown as { readonly n: number };
      base.prepare(`DELETE FROM ${table} WHERE profil_id = ?`).run(profilId);
      comptes.push({ table, lignesEffacees: Number(compte.n) });
    }
 ...
 */
export function tablesNonVidees(
  base: DatabaseSync,
  profilId: string,
  portee: PorteeReinitialisation
): readonly LigneRapportReinitialisation[] {
  return previsualiserReinitialisation(base, profilId, portee).filter(
