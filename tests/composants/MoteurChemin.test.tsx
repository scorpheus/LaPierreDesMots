/**
 * Le moteur `chemin` monté isolément — annexe T § T1 : « un test par moteur pour : bonne
 ...
 * Le harnais ci-dessous referme la boucle avec le VRAI réducteur `moteurChemin.reduire`, de
...
const CHEMIN_HABILLAGE = 'contenu/habillages/clairiere/lianes.habillage.json';
...
function Harnais(): ReactElement {
  const [etat, setEtat] = useState<EtatChemin>(() =>
    moteurChemin.creerEtat({ contenu, habillage, alea, horloge }),
  );
  const emettre = useCallback((action: ActionChemin) => {
    setEtat((precedent) => moteurChemin.reduire(precedent, action, { alea, horloge }));
  }, []);

  const resume = moteurChemin.resume(etat);
  const progression = moteurChemin.progression(etat);
...
describe('moteur chemin', () => {
  ...
  it('mauvaise réponse : une erreur comptée, rien de rouge, la tentative reste réussie', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-case="case-long"]']);
    const h = harnais(container);
