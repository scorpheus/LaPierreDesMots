/**
 * Le moteur `histoire` monté isolément — annexe T § T1 : « un test par moteur pour : bonne
 ...
 * Le harnais ci-dessous referme la boucle avec le VRAI réducteur `moteurHistoire.reduire`, de
...
import { moteurHistoire } from '@partage/moteurs/histoire/moteur';
...
const CHEMIN_HABILLAGE = 'contenu/habillages/clairiere/veillee.habillage.json';
...
function Harnais(): ReactElement {
  const [etat, setEtat] = useState<EtatHistoire>(() =>
    moteurHistoire.creerEtat({ contenu, habillage, alea, horloge }),
  );
  const emettre = useCallback((action: ActionHistoire) => {
    setEtat((precedent) => moteurHistoire.reduire(precedent, action, { alea, horloge }));
  }, []);

  const resume = moteurHistoire.resume(etat);
  const progression = moteurHistoire.progression(etat);
...
describe('moteur histoire', () => {
  it('le contenu de ce test est conforme au schéma que le moteur publie', () => {
    ...
    const valider = ajv.compile(moteurHistoire.schemaContenu);
  ...
  it('mauvaise réponse : une erreur comptée, rien de rouge, la tentative reste réussie', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-option="opt-faux"]']);
    const h = harnais(container);
    ...
    expect(h.getAttribute('data-confusion')).toBe('vrai>faux');
  });

  it('aide de Gobi : exactement le palier `indice`, jamais davantage', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-action="aide"]']);
    expect(container.querySelector('[data-moteur="histoire"]')?.getAttribute('data-aide')).toBe(
      'indice',
    );
  ...
  it('double-tap : le second appui ne compte aucune erreur', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-option="opt-vrai"]', '[data-option="opt-vrai"]']);
