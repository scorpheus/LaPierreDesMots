...
import {
  ...
  INTERRUPTEURS_DE_LECTURE,
} from "@client/ecrans/EcranReglagesLecture";
  ...
  render(
    <QueryClientProvider client={client}>
      <FournisseurJeu valeur={{ services: jeu, magasin }}>
        <EcranReglagesLecture profil={profilDeTest()} surFermeture={() => fermetures.push(1)} />
      </FournisseurJeu>
    </QueryClientProvider>,
  );
}
...
function valeurBrute(champ: string): number {
  return Number(
    document
      .querySelector(`[data-valeur-brute][data-valeur="${champ}"]`)
...
describe("les réglages SE PROPAGENT — les trois maillons de la chaîne", () => {
  it("maillon 0 : l’écran repart des réglages ENREGISTRÉS, jamais des défauts", async () => {
    ...
    expect(
      document.querySelector('[data-choix-police="opendyslexic"]')?.getAttribute("data-choisi"),
    ).toBe("oui");
    expect(
  ...
  it("maillon 1 bis : le pas fractionnaire ne dérive PAS en flottant", async () => {
    ...
    expect(
      String(valeurBrute("interlettrageEm")).length,
      "la valeur a dérivé en flottant",
  ...
  it("maillon 2 : l’APERÇU rendu change avec la valeur — pas seulement le nombre", async () => {
    ...
    console.log(
      `[QA-2 · reglages-lecture] style de l’aperçu — changé : ${String(avant !== apres)}`,
    );
  ...
  it("rien n’est écrit sans être audible : chaque groupe de LECTURE porte son écoute (R15)", () => {
    ...
    expect(document.querySelector('[data-ecouter="titre"]')?.getAttribute("data-ecoutes")).toBe(
      "1",
    );
  });
  ...
  it("rien ne peut être raté ici : aucun bouton « valider », aucun écran d’erreur (R14)", () => {
    ...
    expect(
      document
        .querySelector("[data-etat-enregistrement]")
        ?.getAttribute("data-etat-enregistrement"),
    ).not.toBe("echec");
  });
});
...
describe("l’exigence commune des dix écrans", () => {
  ...
  it("aucune cible tapable ne manque sa déclaration de 64 px (R16)", () => {
    ...
    // 3 polices + 8 moins/plus + 5 écoutes + 4 bascules + le retour : l'écran est dense, et
