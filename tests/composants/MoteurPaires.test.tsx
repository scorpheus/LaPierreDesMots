/**
 * Le moteur `paires` monté isolément — annexe T § T1 : « un test par moteur pour : bonne
 ...
 * Le harnais ci-dessous referme la boucle avec le VRAI réducteur `moteurPaires.reduire`, de
...
const CHEMIN_HABILLAGE = 'contenu/habillages/marais-jumeau/coquillages.habillage.json';
...
function Harnais(): ReactElement {
  const [etat, setEtat] = useState<EtatPaires>(() =>
    moteurPaires.creerEtat({ contenu, habillage, alea, horloge }),
  );
  const emettre = useCallback((action: ActionPaires) => {
    setEtat((precedent) => moteurPaires.reduire(precedent, action, { alea, horloge }));
  }, []);

  const resume = moteurPaires.resume(etat);
  const progression = moteurPaires.progression(etat);
...
describe('moteur paires', () => {
  ...
  it('mauvaise réponse : une erreur comptée, rien de rouge, la tentative reste réussie', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-carte="carte-mot-loup"]', '[data-carte="carte-img-roue"]']);
    const h = harnais(container);
    ...
    expect(h.getAttribute('data-confusion')).toBe('loup>une roue');
  });

  it('aide de Gobi : exactement le palier `indice`, jamais davantage', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-action="aide"]']);
    expect(container.querySelector('[data-moteur="paires"]')?.getAttribute('data-aide')).toBe(
      'indice',
    );
  ...
  it('double-tap : le second appui ne compte aucune erreur', () => {
    const { container } = render(<Harnais />);
    taper(container, [
      '[data-carte="carte-mot-loup"]',
      '[data-carte="carte-img-loup"]',
      '[data-carte="carte-mot-loup"]',
    ]);
