/**
 * Le moteur `phrase` monté isolément — annexe T § T1 : « un test par moteur pour : bonne
 ...
 * Le harnais ci-dessous referme la boucle avec le VRAI réducteur `moteurPhrase.reduire`, de
...
import { moteurPhrase } from '@partage/moteurs/phrase/moteur';
...
const CHEMIN_HABILLAGE = 'contenu/habillages/clairiere/guirlande.habillage.json';
...
function Harnais(): ReactElement {
  const [etat, setEtat] = useState<EtatPhrase>(() =>
    moteurPhrase.creerEtat({ contenu, habillage, alea, horloge }),
  );
  const emettre = useCallback((action: ActionPhrase) => {
    setEtat((precedent) => moteurPhrase.reduire(precedent, action, { alea, horloge }));
  }, []);

  const resume = moteurPhrase.resume(etat);
  const progression = moteurPhrase.progression(etat);
...
describe('moteur phrase', () => {
  ...
  it('le contenu de ce test est conforme au schéma que le moteur publie', () => {
    ...
    const valider = ajv.compile(moteurPhrase.schemaContenu);
    expect(valider(contenu)).toBe(true);
  });

  it('l’habillage lu sur disque déclare bien ce moteur', () => {
    expect(habillage.moteurs).toContain('phrase');
  });

  it('bonne réponse : l’étape avance, aucune erreur, aucun écran d’échec', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-etiquette="etiq-le"]', '[data-etiquette="etiq-loup"]', '[data-etiquette="etiq-dort"]']);
  ...
  it('mauvaise réponse : une erreur comptée, rien de rouge, la tentative reste réussie', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-etiquette="etiq-loup"]']);
    const h = harnais(container);
    ...
    expect(h.getAttribute('data-confusion')).toBe('Le>loup');
  });

  it('aide de Gobi : exactement le palier `indice`, jamais davantage', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-harnais-action="aide"]']);
    expect(container.querySelector('[data-moteur="phrase"]')?.getAttribute('data-aide')).toBe(
      'indice',
    );
  ...
  it('double-tap : le second appui ne compte aucune erreur', () => {
    const { container } = render(<Harnais />);
    taper(container, [
      '[data-etiquette="etiq-le"]',
      '[data-etiquette="etiq-le"]',
      '[data-etiquette="etiq-loup"]',
      '[data-etiquette="etiq-dort"]',
    ]);
