/**
 * Le moteur `chrono` monté isolément — annexe T § T1 : « un test par moteur pour : bonne
 ...
 * Le harnais ci-dessous referme la boucle avec le VRAI réducteur `moteurChrono.reduire`, de
...
const CHEMIN_HABILLAGE = 'contenu/habillages/cite-des-histoires/pellicule.habillage.json';
...
function Harnais(): ReactElement {
  const [etat, setEtat] = useState<EtatChrono>(() =>
    moteurChrono.creerEtat({ contenu, habillage, alea, horloge }),
  );
  const emettre = useCallback((action: ActionChrono) => {
    setEtat((precedent) => moteurChrono.reduire(precedent, action, { alea, horloge }));
  }, []);

  const resume = moteurChrono.resume(etat);
  const progression = moteurChrono.progression(etat);
...
describe('moteur chrono', () => {
  it('le contenu de ce test est conforme au schéma que le moteur publie', () => {
    ...
    const valider = ajv.compile(moteurChrono.schemaContenu);
    expect(valider(contenu)).toBe(true);
  });

  it('l’habillage lu sur disque déclare bien ce moteur', () => {
    expect(habillage.moteurs).toContain('chrono');
  });

  it('bonne réponse : l’étape avance, aucune erreur, aucun écran d’échec', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-vignette="vig-reveil"]', '[data-vignette="vig-sortie"]', '[data-vignette="vig-lune"]']);
  ...
  it('mauvaise réponse : une erreur comptée, rien de rouge, la tentative reste réussie', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-vignette="vig-lune"]']);
    const h = harnais(container);
    ...
    expect(h.getAttribute('data-confusion')).toBe('il se réveille>il regarde la lune');
  });

  it('aide de Gobi : exactement le palier `indice`, jamais davantage', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-action="aide"]']);
    expect(container.querySelector('[data-moteur="chrono"]')?.getAttribute('data-aide')).toBe(
      'indice',
    );
  ...
  it('double-tap : le second appui ne compte aucune erreur', () => {
    const { container } = render(<Harnais />);
    taper(container, [
      '[data-vignette="vig-reveil"]',
      '[data-vignette="vig-reveil"]',
      '[data-vignette="vig-sortie"]',
      '[data-vignette="vig-lune"]',
    ]);
