/**
 * Le moteur `grave` monté isolément — annexe T § T1 : « un test par moteur pour : bonne
 ...
 * Le harnais ci-dessous referme la boucle avec le VRAI réducteur `moteurGrave.reduire`, de
...
const CHEMIN_HABILLAGE = 'contenu/habillages/galeries/pierre.habillage.json';
...
function Harnais(): ReactElement {
  const [etat, setEtat] = useState<EtatGrave>(() =>
    moteurGrave.creerEtat({ contenu, habillage, alea, horloge }),
  );
  const emettre = useCallback((action: ActionGrave) => {
    setEtat((precedent) => moteurGrave.reduire(precedent, action, { alea, horloge }));
  }, []);

  const resume = moteurGrave.resume(etat);
  const progression = moteurGrave.progression(etat);
...
describe('moteur grave', () => {
  ...
  it('aide de Gobi : exactement le palier `indice`, jamais davantage', () => {
    ...
    expect(container.querySelector('[data-moteur="grave"]')?.getAttribute('data-aide')).toBe(
      'indice',
    );
  ...
  it('double-tap : le second appui ne compte aucune erreur', () => {
    const { container } = render(<Harnais />);
    taper(container, ['[data-lettre="b"]', '[data-lettre="b"]']);
